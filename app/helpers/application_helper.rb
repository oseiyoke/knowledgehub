module ApplicationHelper
  ...
  def header_links
    ...
    if @user && @user.can_submit_stories?
      @header_links[new_story_path] = { :title => "Submit Post" }
    end

    if @user
      @header_links[saved_path] = { :title => "Saved Posts" }
    end
    ...
    if @user
      ...
      if (count = @user.unread_message_count) > 0
        @right_header_links[messages_path] = {
          :class => ["new_messages"],
          :title => "#{@user.unread_message_count} Message".pluralize(count) + "/Notifications", 
        }
      else
